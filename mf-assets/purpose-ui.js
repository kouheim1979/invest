...
(function(root){
  ...
  function create(ctx){
    ...
    function page(){
      ...
      const automatic=new Set(s.meClassification?.suggestedKeys||[]),suggested=holdings.filter(h=>automatic.has(h.key)&&summary.assignment.has(h.key)).length;
      ...
      return '<div id="purpose-page" class="purpose-page">'+
        ...
        (suggested?'<div class="callout" id="me-classification-note"><b>'+suggested+'資産を仮分類しています</b><p>口座名義と資産区分から分類しました。株・投信は資産形成、年金は老後資金、子ども名義は子ども名義資金、預金は待機資金として表示します。実際の使い道に合わせて下の一覧から変更できます。</p></div>':'')+
    ...
    function assetList(rows,settings,assignment){
      ...
      const visible=found.slice(0,150),suggestions=new Set(state().meClassification?.suggestedKeys||[]);
      return visible.map(h=>'<div class="purpose-asset-row"><div class="purpose-asset-info"><b>'+esc(h.name||h.symbol)+'</b><small>'+esc(C.ownerOf(h,state()))+' · '+esc(h.account)+'</small><small>'+esc(h.asOf)+' 時点</small>'+(suggestions.has(h.key)?'<small>目的は仮分類 · 変更できます</small>':'')+'</div><div class="purpose-asset-controls"><strong class="money">'+yen(h.valueJPY)+'</strong><label><span class="sr-only">'+esc(h.name||h.symbol)+'の目的</span><select aria-label="'+esc(h.name||h.symbol)+'の目的" data-purpose-holding="'+esc(h.id)+'">'+options(settings.groups,assignment.get(h.key)||P.UNASSIGNED)+'</select></label></div></div>').join('')+(found.length>150?'<p class="purpose-footnote">'+found.length+'件のうち先頭150件を表示しています。検索で絞り込めます。</p>':'');
    }
    function editor(){
