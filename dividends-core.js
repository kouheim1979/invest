...
function merge(existing,incoming){const next=existing.map(r=>({...r}));const keys=new Map(next.map(r=>[r.id?'id:'+r.id:'row:'+fingerprint(r),r]));let skipped=0;for(const r of incoming){const k=r.id?'id:'+r.id:'row:'+fingerprint(r);if(keys.has(k)){if(fingerprint(keys.get(k))!==fingerprint(r))throw Error('同じ明細IDで内容が異なります: '+r.id);skipped++;continue;}next.push(r);keys.set(k,r);}return{receipts:next.sort((a,b)=>b.date.localeCompare(a.date)),added:next.length-existing.length,skipped};}
