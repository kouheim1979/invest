...
While publication is paused, run `python3 everyday-japan-lab/scripts/build.py --manifest-only` to refresh the export content without changing any preview HTML or assets. Then run `python3 everyday-japan-lab/scripts/export_blogger.py` to create the self-contained Blogger update pack. The private Blogger site is `https://everydayjapanlab.blogspot.com/`; start with `/p/home.html`. All 23 pages and the shared tool gadget are saved behind authors-only access, and their actual URLs are recorded in `deployment/published-pages.json`. Reuse those pages when updating them.
