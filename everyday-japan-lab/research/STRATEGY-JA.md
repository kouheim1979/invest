> 2026-09-25追記：Blogger本番は本人の指示で一般公開を再開。検索表示は自動承認レビューの拒否によりOFFのまま。開発版は休止を維持。サイト表記はEveryday Japan Lab、Contactは本人指定のサイト用メール。受信・返信時の表示名は未確認。現在の状態はdeployment/STATUS-JA.mdを参照。
