...
def mapped(body,mapping):
    return re.sub(r'href="'+re.escape(BASE)+r'([^"#?]*)([^\"]*)"',lambda m:'href="'+mapping.get(m.group(1).strip('/'),m.group(0))+m.group(2)+'"',body)
...
for p in pages:
    ...
    menu='<nav class="ejl-menu" aria-label="Everyday Japan Lab">'+''.join(f'<a href="{urlmap[s]}">{t}</a>' for s,t in [('','Home'),('topics','Topics'),('tools','Tools'),('about','About'),('contact','Contact'),('privacy','Privacy')])+'</nav>'
    intro='' if not slug else f'<p class="eyebrow">{escape(p["tag"])}</p><p class="lede">{escape(p["description"])}</p><p class="meta">By Everyday Japan Lab · Reviewed 2026-09-24</p>'
    fragment='<div class="ejl-fragment" lang="en">'+menu+intro+mapped(body,urlmap)+'</div>'
    ...
    preview=f'<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="robots" content="noindex,nofollow"><title>{escape(p["title"])}</title><style>{css}</style></head><body class="ejl"><main class="wrap"><header class="page-head"><h1>{escape(p["title"])}</h1><p>{escape(p["description"])}</p></header>{previewbody}</main><script>{js}</script></body></html>'
    (OUT/'preview'/f'{name}.html').write_text(preview)
    manifest.append(dict(file=f'pages/{name}.html',title=p['title'],suggested_path=urlmap[slug],confirmed=False))
...
アカウントや広告設定は作成していません。Blogger実環境での動作確認は、ブログURL確定後に必要です。

1. Bloggerで無料blogspotブログを本人が作成。検索公開は移植・確認後に有効化。
...
print(f'Exported {len(pages)} Blogger page fragments, common gadget and offline previews; actual URL mapping pending.')
