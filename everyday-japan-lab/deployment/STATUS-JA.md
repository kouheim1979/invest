# 独自の誌面デザインへ更新 — 2026-09-25（日本時間）

本番トップとHome固定ページを、紙面色・深緑・赤茶を基調とした実用誌のデザインへ更新。汎用の山の背景を非表示にし、自作SVGの家の断面図と、図中の01〜03に対応したbidet / hot water / preparednessの入口を追加した。5ツールを目的別の一覧にし、調査方針と広告・Privacyへのリンクを残した。

- 共通HTMLガジェットと既存HomeページをBlogger管理画面で保存。記事・計算モデルを変更せず、共通の文字・余白・フォーム外観を調整した。
- ソース: `deployment/design/editorial-home.html`, `editorial.css`, `front-page.js`。`scripts/export_blogger.py`が移植用ページとガジェットへ組み込む。
- トップでは静的テンプレートをJavaScriptでmainへ配置。記事ページではテンプレートは不活性。JSが実行されない場合は既存紹介投稿が残り、Home固定ページには新しい本文を直接保存してある。
- 公開トップとHomeのDOM、canonical、本文重複なし、デスクトップの横はみ出しなし、対象個人名パターンの一致なしを確認。
- トップの導線から備蓄ツールへ移動し、4人×7日×3 L=84 L、在庫10 Lなら追加74 L・2 L容器37個を確認。
- Blogger標準のスマートフォンプレビューで見出しの改行・図の幅を確認。実機のiOS Safari検証ではない。ローカルfile URLでの補助検証はブラウザーのURLポリシーで拒否されたため利用していない。
- 新ホームのリンク先は既存の公開ページ一覧と照合済み。数式、検索公開、AdSense審査中、開発版休止の状態を維持。
- 画面確認によるアクセスは集客実績に含めて評価しない。
