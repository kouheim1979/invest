...
GitHub PagesのEveryday Japan LabとJapan Ahead旧URLは休止案内へ置換する。生成元は保持し、`preview_paused: true` で通常ビルドによる再公開を防ぐ。GitHubリポジトリと履歴そのものは公開のままで、この変更はリポジトリの非公開化や検索エンジンの既存キャッシュ削除ではない。再開は本人の指示後に行う。
...
- 先に名前表記・Contactの連絡窓口を修正する。公開停止は匿名化の完了を意味しない。
