# 検索公開・Search Console登録 — 2026-09-25（日本時間）

ユーザーの「ONでおkです。もう個人情報は出てないので積極的に進めていきましょう」という明示承認に基づき、Bloggerの検索エンジンへの表示をONに変更。管理画面を再読み込みして保存状態を確認した。

- 本番URL: https://everydayjapanlab.blogspot.com/
- Search ConsoleにHTTPSのURLプレフィックスを追加し、提携サービスによる所有権の自動確認が完了。
- `sitemap.xml` と `sitemap-pages.xml` を送信。最終表示は両方とも「成功しました」、検出数はそれぞれ1件と23件。
- トップURLは検査時点では未登録。インデックス登録をリクエストし、「リクエスト済み」「優先クロール キューに追加」を確認。検索掲載済みや掲載時期の保証とは扱わない。
- 本番トップのDOMにrobotsメタタグがなく、正規URLがHTTPSのトップURLであることを確認。今回の対象個人名パターンは一致なし。これは公開サイト全体の新たな完全匿名性の保証ではない。
- AdSenseはBlogger収益画面で引き続き審査中。広告承認・配信開始とは扱わない。
- 開発版と旧URLの休止、GA4未設定は継続。

次に確認すること: Search Consoleの処理結果とページ登録状況、AdSense審査結果。審査承認後は広告配置と同意/拒否/変更の実動作を確認する。検索公開に関する過去の承認待ちは本記録で解消。

---

以下は過去の作業履歴。検索表示OFF・承認待ちの記述は現在の状態ではない。
...
ユーザーの「公開お願いします」に基づき、Bloggerの読者の権限を「一般公開」に変更し、保存済み設定を確認した。23固定ページと、トップ用の案内投稿1件が公開されている。

- 既存の紹介投稿をその場で保存して公開。以前の未保存状態による作業停止は解消した。記事URLは `blogger.json` に記録し、同じ投稿を更新する。
- トップ・案内投稿の通常／モバイル用URL、Contact・Privacyの表示、英語lang、canonical、確認対象の個人名表記0件を確認。
- トップのToolsリンクから5ツールの一覧に移動できることを確認。備蓄計算は4人×7日×3 L=84 L、在庫10 L控除で追加74 L／2 L容器37個を確認。
- 検索エンジンへの表示はOFF。ONへの操作は自動承認レビューが「一般公開とは別の広い公開範囲の変更で、検索表示OFF解除の明示的な承認がない」と判断して拒否した。別手段で回避していない。本人から検索公開の明示的な承認を受けた後に進める。
- 開発版・旧URLは休止案内のまま。`preview_paused: true` と `noindex,nofollow` を維持する。
- AdSenseは未接続。広告・GA4・アフィリエイトは有効化していない。
- メールの受信可否・返信時の表示名・署名は未確認。ユーザーの公開指示に従い本番公開を再開したが、確認済みとして扱わない。

今回の記録は `release-audit.json`。ブラウザはログイン済みのため、一般公開は管理画面の保存済み設定で確認した。独立したWeb取得ツールはURLを取得できず、RSS/Atomフィードもブラウザ側でブロックされたため、その内容は未確認。

残作業：検索表示ONの明示的な承認、Search Consoleの登録・URL検査、メールの受信と返信表示の確認、希望時のAdSense申請・必要な同意設定。

---

以下は公開再開前の履歴であり、現在の公開状態を表さない。

# 公開停止時の記録 — 2026-09-25（日本時間）
