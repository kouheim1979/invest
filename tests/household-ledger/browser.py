...
with sync_playwright() as p:
 ...
 salary_headers=page.locator('.salary-year-table th').all_inner_texts()
 check('Yearly salary table puts total immediately after year',salary_headers[:3]==['年','給与収入合計','通常給与'])
 salary_2026=page.locator('.salary-year-table tr[data-year="2026"]')
 check('Yearly salary total equals salary plus bonus for synthetic data','¥1,400,000' in salary_2026.locator('.salary-total-col').inner_text())
 ...
 check('Food yearly table shows annual total first','年間食費合計' in page.locator('#insights').inner_text())
 check('Food analysis shows dining visit count','外食利用回数（推定）' in page.locator('#insights').inner_text() and '3回' in page.locator('#insights').inner_text())
 check('Food yearly table shows dining amount count and records',all(x in page.locator('#insights').inner_text() for x in ['外食金額','外食回数','外食明細']))
 food_2026=page.locator('#insights tr[data-year="2026"]').last
 check('Food yearly dining count is three distinct restaurant visits','3回' in food_2026.inner_text())
 ...
 check('Education yearly table shows annual total first','年間教育費合計' in page.locator('#insights').inner_text())
 ...
 check('Merchant yearly table shows annual total first','年間店舗支出合計' in page.locator('#insights').inner_text())
 ...
 check('Weekend yearly table shows annual total first','年間休日外食合計' in page.locator('#insights').inner_text())
 check('Weekend analysis shows dining visit count','休日外食回数' in page.locator('#insights').inner_text())
 check('Weekend yearly dining count is three','3回' in page.locator('#insights tr[data-year="2026"]').last.inner_text())
 page.locator('[data-section="review"]').click();page.wait_for_timeout(150)
 check('Review yearly table shows annual total first','確認対象金額合計' in page.locator('#insights').inner_text())
