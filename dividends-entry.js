...
const tabs=document.querySelector('.tabs');if(tabs){const link=document.createElement('a');link.id='dividendNav';link.className='tab dividend-nav';link.href='./dividends.html?v=20260916-1';link.textContent='配当10年';tabs.appendChild(link);}
let queued=false;
function decorate(){queued=false;document.querySelectorAll('#body .chart-btn,#mobile .chart-btn').forEach(button=>{if(button.parentElement.querySelector('.dividend-link'))return;const match=String(button.getAttribute('onclick')||'').match(/openChart\('([0-9A-Z]{4,6}\.[TS])'\)/);if(!match)return;const a=document.createElement('a');a.className='edit dividend-link';a.href='./dividends.html?symbol='+encodeURIComponent(match[1]);a.textContent='配当10年';a.setAttribute('aria-label',match[1]+'の10年間の配当');button.insertAdjacentElement('afterend',a);});}
