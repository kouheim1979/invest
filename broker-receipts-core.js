...
function validate(raw){
 ...
 const documents=array(raw.documents,1000).map(d=>({id:nonempty(d.id),name:nonempty(d.name),kind:['pdf','csv'].includes(d.kind)?d.kind:fail('資料形式が不明です。'),account:nonempty(d.account),from:date(d.from),to:date(d.to),coverageFrom:date(d.coverageFrom),coverageTo:date(d.coverageTo),note:str(d.note)}));
 ...
 const receipts=array(raw.receipts,20000).map(r=>{
  ...
  for(const s of row.sources){const d=byId.get(s.documentId);if(!d||d.account!==row.account||row.date<d.from||row.date>d.to)fail('明細と出典の口座・期間が一致しません。');if(s.page===null&&s.line===null)fail('資料内の位置がありません。');const pos=JSON.stringify(s);if(positions.has(pos))fail('同じ資料位置が重複しています。');positions.add(pos);}
  return row;
 });
 return{schema:raw.schema,version:1,preparedAt:date(raw.preparedAt),scopeNote:str(raw.scopeNote,4000),documents:documents.sort((a,b)=>a.from.localeCompare(b.from)||a.id.localeCompare(b.id)),receipts:receipts.sort((a,b)=>a.date.localeCompare(b.date)||a.id.localeCompare(b.id))};
}
function merge(old,incoming){
 ...
 const notes=[...new Set([a.scopeNote,p.scopeNote].filter(Boolean))].join('\n');
 return validate({...p,preparedAt:a.preparedAt>p.preparedAt?a.preparedAt:p.preparedAt,scopeNote:notes,documents:[...docs.values()],receipts:[...rows.values()]});
}
...
function ranges(p,account=''){const spans=p.documents.filter(d=>!account||d.account===account).map(d=>({from:d.coverageFrom,to:d.coverageTo})).sort((a,b)=>a.from.localeCompare(b.from));const out=[];for(const s of spans){const last=out.at(-1);if(last&&Date.parse(s.from)<=Date.parse(last.to)+86400000){if(s.to>last.to)last.to=s.to;}else out.push({...s});}return out;}
function coverage(p,from,to,account=''){
 const accounts=[...new Set(p.documents.filter(d=>!account||d.account===account).map(d=>d.account))];
 if(!accounts.length)return 'unknown';
 const states=accounts.map(a=>{const spans=ranges(p,a);return spans.some(s=>s.from<=from&&s.to>=to)?'full':spans.some(s=>s.from<=to&&s.to>=from)?'partial':'unknown';});
 return states.every(s=>s==='full')?'full':states.every(s=>s==='unknown')?'unknown':'partial';
}
function years(p,account=''){const ds=p.documents.filter(d=>!account||d.account===account);if(!ds.length)return[];const start=Math.min(...ds.map(d=>+d.coverageFrom.slice(0,4))),end=Math.max(...ds.map(d=>+d.coverageTo.slice(0,4)));const out=[];for(let y=start;y<=end;y++){const rows=p.receipts.filter(r=>(!account||r.account===account)&&r.date.startsWith(y+'-'));out.push({year:y,...total(rows),coverage:coverage(p,y+'-01-01',y+'-12-31',account)});}return out;}
function months(p,year,account=''){return Array.from({length:12},(_,i)=>{const from=year+'-'+String(i+1).padStart(2,'0')+'-01',to=new Date(Date.UTC(+year,i+1,0)).toISOString().slice(0,10);return{month:i+1,...total(p.receipts.filter(r=>(!account||r.account===account)&&r.date.startsWith(from.slice(0,7)))),coverage:coverage(p,from,to,account)};});}
...
return{KEY,labels,validate,merge,total,ranges,coverage,years,months,read,save,undo};
});
